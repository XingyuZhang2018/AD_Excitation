...
function find_groundstate(model::HamiltonianModel, 
    ...
    out_log_file = joinpath(outfolder,"idmrg1_mps_$(Ni)x$(Nj)_D$(D2)_χ$(targχ).log")
    ...
    while err > alg.tol && i < alg.maxiter
        ...
        if4site && (energy /= 4)
        if2site && (energy /= 2)
        message = "$t idmrg1@$i err = $err energy = $(energy)\n "
    ...
    verbose && println("idmrg1 done@$i err = $err energy = $(energy)")
    return energy
end
...
function find_groundstate(model::HamiltonianModel, 
    ...
    outfolder = joinpath(outfolder, "$model", "groundstate")
    out_chkp_file = joinpath(outfolder,"canonical_mps_$(Ni)x$(Nj)_D$(D2)_χ$(targχ).jld2")
    out_log_file = joinpath(outfolder,"idmrg2_mps_$(Ni)x$(Nj)_D$(D2)_χ$(targχ).log")
    ...
    while err > alg.tol && i < alg.maxiter
        ...
        AC2 = ein"abcij,cdij,defij->abefij"(AL, C, AR)
        ...
        # ELL, ƎRR = envir_MPO(AL, AR, M) # better energy estimate
        AC = ALCtoAC(AL, C)
        e2 = ein"(((adfij,abcij),dgebij),cehij),fghij -> "(E,AC,M,Ǝ,conj(AC))[]
        e1 = ein"((abij,acdij),bceij),deij->"(C,E,Ǝ,conj(C))[]
        energy = real(e2 - e1)
        if4site && (energy /= 4)
        if2site && (energy /= 2)
        message = "$t idmrg2@$i err = $err energy = $(energy)\n "
        verbose && (i % alg.show_every) == 0 && print(message)
        logfile = open(out_log_file, "a")
        write(logfile, message)
        close(logfile)
    ...
    save(out_chkp_file, "ALCAR", map(Array, (AL, C, AR)))
    verbose && println("idmrg2 done@$i err = $err")
    return energy
end